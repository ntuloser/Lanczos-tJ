	...
	for (int stp=0;stp<Variational_steps ; stp++) {
        ...
        for (int steps=0;steps<=totsteps;steps++){
            ...
            if (steps%interval==0 && steps>warmup){
                ...
                Traversal(0,1, config_level, a_ij, slater, deter_origin, deriv_D, deriv_Mu, &tot_E, &tot_O_DtimesE, &tot_O_MutimesE, &temp_EperSample,Energy_level,tot_E_power);
        ...
        double SUM_tot_E_power[4]={0,0,0,0};
        ...
        MPI_Reduce(tot_E_power,SUM_tot_E_power,4,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD);
        ...
        if (mynode==0) {//This is the master branch
            ...
            cout<<"Epower0 ="<<tot_E_power[0] /pow(SIZE,2)/sample<<endl;
            cout<<"Epower1 ="<<tot_E_power[1] /pow(SIZE,4)/sample<<endl;
            cout<<"Epower2 ="<<tot_E_power[2] /pow(SIZE,6)/sample<<endl;
